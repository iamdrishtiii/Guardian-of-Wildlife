...
const Program = () => {
  ...
  const prevSlide = () => {
    setCurrentSlide(
      (prev) => (prev - 1 + currentPrograms.length) % currentPrograms.length
    );
  };
    ...
    <div className="pb-[450px] 3xl:pb-48 md:pb-64">
      ...
      {currentPrograms.length > 0 && (
        <div className="relative w-full flex justify-center items-center sm:p-6 mt-4 ">
          <button
            onClick={prevSlide}
            className="absolute left-0 md:left-10 p-2 bg-gray-300 rounded-full"
          >
            ◀
          </button>
          <Link
            to={`/programs/${currentPrograms[currentSlide].num}/${currentPrograms[currentSlide].ProgramName}`}
          >
            <div className="w-[300px] sm:w-[500px] h-[300px] px-8 flex justify-center items-center flex flex-col">
              ...
              <p className="text-lg md:text-2xl font-semibold">
                {currentPrograms[currentSlide].ProgramName}
              </p>
            </div>
          </Link>
          <button
            onClick={nextSlide}
            className="absolute right-0 md:right-10 p-2 bg-gray-300 rounded-full"
          >
            ▶
          </button>
        </div>
          ...
          {currentPrograms.map((program, index) => (
            <div key={program.num}>
              <Link to={`/programs/${program.num}/${program.ProgramName}`}>
                <div
                  className={`md:flex flex-row hover:bg-gray-100 p-6 rounded-xl ${
                ...
                >
                  ...
                  <div className="md:w-2/3 p-4 text-xl md:text-2xl">
                    <p className="w-full h-auto rounded-lg font-bold text-2xl md:text-4xl ">
                      {program.ProgramName}
                    </p>
                    <br />
